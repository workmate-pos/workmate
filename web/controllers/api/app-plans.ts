...
import { AppPlan, appPlans } from '../../services/app-plans.js';
...
export default class AppPlans {
  @Get('/')
  async getAvailableAppPlans(req: Request, res: Response): Promise<Response> {
  ...
  @Get('/subscription')
  async getAppPlanSubscription(req: Request, res: Response): Promise<Response> {
    const session = res.locals.shopify.session;
    const graphql = new Graphql(session);
    const appPlanSubscription = await appPlans.getAppPlanSubscription(graphql);
  ...
  async createNewAppPlan(req: Request<unknown, unknown, CreateAppPlanSubscription>, res: Response): Promise<Response> {
    const session = res.locals.shopify.session;
    const shop = session.shop;
    const graphql = new Graphql(session);
    ...
    if (appPlan == null) return res.status(400).json({ error: 'App plan not found' });
