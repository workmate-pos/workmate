...
export type BatcherFetchResult<B extends (...args: any) => any> =
  ReturnType<B> extends { fetch: (param: infer P) => Promise<infer R> } ? R : never;

/**
 * Exposes a `fetch` function that aggregates all calls into a single batched call.
 * Uses a `key` to globally cache batches, similar to react-query.
...
export const useBatcher = <Param, Result, BatchParam = undefined>({
  ...
  const batch: Batches[keyof Batches] = (batches[key] ??= { queue: [], timer: undefined });
